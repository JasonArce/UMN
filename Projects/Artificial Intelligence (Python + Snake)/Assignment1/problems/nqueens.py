...
class NQueens(Problem):
    ...
    def prune(self, state):
        # MODIFY this so that it does not generate actions for those values
        # that violate the constraint that no two queens are in the same column
        # You do not need to prune actions that violate the diagonal constraint.

        #added to check if the state is unqiue. If so(no violations in columns), return size. Else, return default action.
        if all_unique(state):
            return range(self.size)
        else:
            actions = []
