...
type arithToken = PLUS | TIMES | DIV | NEG | CONST of float
...
let e2 = AddExpr(ConstExpr 2.5, MultExpr(ConstExpr 10.1, ConstExpr 1.3))

let divresult = arithExpEval (rpnParse (token_list ("1 2 /")))

let divresult = arithExpEval (rpnParse (token_list ("6 9 /")))
