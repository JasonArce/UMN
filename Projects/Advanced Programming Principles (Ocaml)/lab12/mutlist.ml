...
let rec exclude_m p ml = match ml with
  | Nil -> ml
  | C mc -> if p mc.hd then let () = mc.hd <- (match mc.tl with Nil -> Nil | C md -> md.hd) in let () = mc.tl <- (match mc.tl with Nil -> Nil | C md -> md.tl) in exclude_m p mc.tl else exclude_m p mc.tl
