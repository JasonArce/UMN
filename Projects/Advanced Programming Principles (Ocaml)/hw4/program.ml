type expr =
  ...
  | Apply of expr * expr
  | Print of expr
  | Readint (* takes no arguments *)
 and expType = Int | Bool | Unit | Other |  FunType of expType * expType (* Other is added for use in parser matching *)
...
let rec eval exp state = match exp with
  ...
  | Print e -> let (r,st') = eval e state in
		 ...
		 | FunResult _ -> print_string "<fun>" in
	       ...
	       let () = flush stdout in
	       (UnitResult, st')
  | Readint -> let n = (read_int()) in (IntResult n, state) (* calls built in read_int() function *)
...
let rec typeof exp env = match exp with
  ...
  | Readint -> Int (* Readint just returnes an Int *)
